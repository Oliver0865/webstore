...
public class ProcessingTimeLogInterceptor implements HandlerInterceptor {
	...
	public void postHandle(HttpServletRequest request, HttpServletResponse response, Object handler,
		...
		String queryString = request.getRequestURL() == null ? "" : "?" + request.getQueryString();
		String path = request.getRequestURL() + queryString;
		long startTime = (Long) request.getAttribute("startTime");
		long endTime = System.currentTimeMillis();
		LOGGER.info(String.format("%s millisecond taken tp process the request %s.", (endTime - startTime), path));
	}
